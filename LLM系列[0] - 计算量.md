...
* 此外, 每个神经元还要加上一个不受前一层神经元影响的常数, 这个常数称为偏置。

因为相邻神经元之间都存在由箭头表示的连接,所以这个网络被称为**全连接网络**。
...
如式所示,Softmax 函数的分子是第 $k$ 类的得分 $s_k$ 的指数函数,分母是所有类别得分的指数函数之和。这部分的计算量相比于线性变换的计算量可以忽略不计。
...
1. 线性变换(矩阵乘法)的计算量为 $2NM$ 或 $2BNM$, 其中 $N$ 是输入维度,$M$ 是神经元个数,$B$ 是批次大小。
2. 非线性变换(激活函数)的计算量相对于线性变换可以忽略不计。
3. Softmax 函数和交叉熵损失函数的计算量也相对于线性变换可以忽略。

[ 特别地,对于 Transformer 这样的大模型,由于其隐藏层维度 $H$ 通常很大,因此全连接层的计算复杂度 $2LH^2$ 也很高,这也是 Transformer 计算量大的主要原因之一 ]
...
( 从这里, 可以看出来, 如果反向传播的时候是需要前向激活的值 (这里 $x$, $y$ 即为前向传播的激活值) , 所以需要记录很多的前向传播的值花费很多的GPU资源 )
...
( 这里又可以看出, 除了当前的参数值$W$, 依然需要使用前向激活值$x$ )
...
如果考虑批量大小为 $B$ 的情况,即 $\mathbf{X} \in \mathbb{R}^{B \times N}, \mathbf{H} \in \mathbb{R}^{B \times M}$,那么反向传播的计算次数为 $4BNM$。可见,批量大小同样是影响反向传播计算代价的一个重要因素。 
...
1. 这里的 $M$ 指当前神经元个数, 这里的 $N$​ 指的是广义的输入层, 即输入当前层上层的输出的维度.  NM 实际上为 一层全连接神经网络的参数量. 

4. 综上, 可得 : **全连接神经网络中前向传播和反向传播的需要的计算资源需要6倍参数量的基本计算(加法或乘法).** 
...
3. 将 $\operatorname{softmax}$ 的结果与 $\mathbf{V}_h$ 相乘。同样的, 这需要 $2NL^2D_H$ 次加乘运算

将这三个步骤的计算次数相加, 得到计算一个头的Attention需要约 $4NL^2D_H$ 次加乘运算 
...
我们假设词表大小为 $V$ , 矩阵乘法(线性变换)的输入为 $NLD$ 和 $DV$  , 则这部分的计算量为 $2NLDV$
...
这里的计算相当于我们只考虑线性层计算, 回忆下前面说的全连接神经网络 这时我们可以将计算量近似估计为每个Token的6倍参数量(一个Epoch)。
...
假设Transformer Blocks的数量为 $N$​, 每个Block内部包含一个 Multi-head Self-attention 层和一个 Feed forward 层。
...
这样可以在保证训练稳定性的同时减少显存占用。

② **梯度累加:**

将多个小批次的梯度累加起来,然后再进行一次参数更新。这样可以在减少显存占用的同时增大有效的批大小。
...
混合精度训练是一种结合了 float32 和 float16 两种数据类型的训练方法, 旨在**加速训练过程**, **节省显存占用**, 同时**保持数值稳定性和模型性能**。
...
a. 前向传播:
   ...
   - 以 float16 精度执行前向传播, 计算模型的输出和损失函数。
   - 将前向传播过程中的 activations 以 float16 精度存储, 以节省显存。
...
#### ① 显存占用 - 模型参数 反向梯度 优化器状态 

在一次训练迭代中，每个可训练模型参数都会对应1个梯度，并对应2个优化器状态（Adam优化器梯度的一阶动量和二阶动量）。

设模型参数量为 Φ ，那么梯度的元素数量为 Φ ，AdamW优化器的元素数量为 2Φ 。
...
> 这里, 使用 混合精度训练确实会使 这部分的显存占用 从 $2 + 2 + 4 + 4 = 12 $ 变到 20 , 但是主要是通过这种方式节省了显存的占用.  
...
对于layer normalization，计算梯度时需要用到层的输入、输入的均值 $μ$ 和方差 $σ^2$ 。输入包含了 $NLD$ 个元素，而输入的均值和方差分别包含了 $NL$ 个元素。由于 ℎ 通常是比较大的（千数量级）。因此，对于layer normalization，中间激活近似估计为 $NLD$

##### 1 Self-Attention 的激活值计算
...
5. 上述 234 步 使用的显存是一个头的, 总共有 5NLLa的显存占用
...
综上, 每个 Encoder 层/ decoder层 需要保存的激活值大小可近似为 $$ , 

也就是说, 与 分头数量 隐藏层维度 以及输入长度 以及 Batchsize1有关

### 2.2.4 对比中间激活与模型参数的显存大小
...
### 3.3.1 流水线并行 [Pipeline Parallelism]
...
$$
需要注意的是, 在混合精度训练时, Megatron存储的梯度并不是半精度的,
